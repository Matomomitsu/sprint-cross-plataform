...
final class BrowserCameraRecorder implements SegmentRecorder {
  ...
  Future<void> initialize() async {
    ...
    for (final device in _ordered(devices)) {
      try {
        ...
        failures.add(
          '${device.label.isEmpty ? device.deviceId : device.label}: $error',
        );
      }
    }
  ...
  Future<RecordedVideo> stop() async {
    ...
    if (_chunks.isEmpty) {
      throw StateError('the recorder produced no video data');
    }
    final blob = web.Blob(
  ...
  Iterable<web.MediaDeviceInfo> _ordered(List<web.MediaDeviceInfo> devices) {
    ...
    final matching = devices.where(
      (d) => d.label.toLowerCase().contains(wanted),
    );
  ...
  int _videoBitrate() {
    ...
    return (rate * _bitsPerFrame).round().clamp(
      _minimumBitrate,
      _maximumBitrate,
    );
  }
  ...
  static Future<web.MediaStream> _open(
    web.MediaStreamConstraints constraints,
  ) async => (await web.window.navigator.mediaDevices
